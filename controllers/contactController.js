...
exports.submitContact = async function (req, res, next) {
  ...
  try {
    const promise1 = transport.sendMail({
      ...
      <p>We appreciate your interest in ${doesPetExist.name} and one of our staff members will contact you shortly! Below is a copy you sent us for your personal records:</p>
      <p><em>${ourObject.comment}</em></p>`
    })

    const promise2 = transport.sendMail({
      to: "petadoption@ok.com",
      from: "petadoption@ok.com",
      ...
      Interested In: ${doesPetExist.name}<br>
